...
template <typename DType, typename IdType>
torch::Tensor UVAIndexSelect_(torch::Tensor input, torch::Tensor index) {
...
torch::Tensor UVAIndexSelect(torch::Tensor input, torch::Tensor index) {
  return AT_DISPATCH_INDEX_TYPES(
      index.scalar_type(), "UVAIndexSelect", ([&] {
        ...
        return GNNDLP_DISPATCH_ELEMENT_SIZES(
            aligned_access_size, "UVAIndexSelectElementSize", ([&] {
              return UVAIndexSelect_<element_size_t, index_t>(input, index);
            }));
      }));
}
